...
public class SentarEspectadores {
	...
	// Array de String para guardar solo Espectadores que cumplan la condicion de
	// Edad y Dinero.
	private String espectIngresa[] = new String[salaOcupada.length * salaOcupada[0].length]; // length the array es x *
																								// y de matriz sala;

	// Metodo para crear la distribucion de los asientos de la sala usando como
	// parametro la instancia de la Clase Sala
	...
	// Metodo para guardar el nombre del Espectador que cumplio la condicion de
	// dinero y edad
	...
	public void guardarNombre(CreacionEspectadores creacionEspectadores, Cine cine, CreacionPelicula nuevaPelicula) {
		...
		for (int i = 0; i < creacionEspectadores.getEspectadores().length; i++) {
			creacionEspectadores.getEspectadores()[i].pagarEntrada(cine, nuevaPelicula); // funcion de Espectador para
																							// comprobar si tiene edad y
																							// dinero
			if (creacionEspectadores.getEspectadores()[i].isPuedeIngresar() && y < espectIngresa.length) {// si es true
																											// y la sala
																											// tiene
																											// lugar se
																											// asigna al
...
//Metodo para ocupar los asientos. Utiliza como argumentos a Sala y CreacionEspectadores 
	public void ocuparAsientos(Sala sala1, CreacionEspectadores creacionEspectadores) {
		...
		while (i < espectIngresa.length) {
			...
			char inicial; // Variable para guardar letra inicial del nombre del espectador (mas prolijo
							// para imprimir)

			if (espectIngresa[i] == null) {
				espectIngresa[i] = "-"; // funcion para modificar el valor null por caracter "-" en caso de que el array
										// tenga nulls.
			}
			inicial = espectIngresa[i].charAt(0); // se guarda la inicial de espectador

			if (salaOcupada[x][y] == null) { // si el asiento de salaOcupada esta vacio(null) se asigna un espectador en
												// esa posicion.
				if (espectIngresa[i] != "-") {
		...
		// Se imprime la sala con la inicial de los espectadores o con "-" si el asiento
		// esta vacio.
		System.out.println("\n----ASIENTOS OCUPADOS----");
