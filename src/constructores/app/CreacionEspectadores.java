...
public class CreacionEspectadores {
	...
	// Esta clase fue creada para crear los espectadores y asignarlos a un array de
	// objetos que luego usare para sentarlos.
	...
	public void crearEspectadores() {
		try { // Try para ver si el valor ingresado por el cliente es correcto, sino saltara
				// mensaje de error.
			Scanner sc = new Scanner(System.in);
			System.out.println("Cuantos espectadores van a intentar ingresar a la sala?");
			int cantidadEspectadores = sc.nextInt(); // Pide numero entero por consola para ver cuantos espectadores se
														// crearan

			espectadores = new Espectador[cantidadEspectadores]; // El largo del array es la cantidad de espectadores
																	// que crearan.

			// Bucle para crear las instancias de objeto con los parametros solicitados.
			// Nombre edad dinero booleano.
		...
		} catch (Exception e) { // Mensaje de error y se reinicia el programa si ingresa algo distinto a numero
								// entero.
...
//			System.exit(0);
	...
	public void imprimirEspectadores() {
		...
		for (Espectador num : espectadores) {
			System.out.println("Nombre: " + num.getNombre() + " - Edad: " + num.getEdad() + " - Dinero: "
					+ num.getDinero() + " - " + num.isPuedeIngresar());
		}
	}
}
